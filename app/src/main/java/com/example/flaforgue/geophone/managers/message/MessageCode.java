package com.example.flaforgue.geophone.managers.message;

/**
 * Created by Baptite_Portable on 12/12/2016.
 */

/**
 * Centralisation des codes des messages gérés
 */
public final class MessageCode {

    public final static String LOCATION_REQUEST = "[GEOPHONE]GET_LOCATION";
    public final static String SEND_LOCATION_CLOSE = "[GEOPHONE]SEND_LOCATION_CLOSE";
    public final static String SEND_LOCATION_FAR = "[GEOPHONE]SEND_LOCATION_FAR";
    public final static String UNKNOWN_LOCATION = "[GEOPHONE]UNKNOWN_LOCATION";
    public final static String ERROR_PIN = "[GEOPHONE]ERROR_PIN";
    public final static String SIMPLE_LOCATION = "[GEOPHONE]SIMPLE";
    public final static String MAPS_LINK = "[GEOPHONE]MAPS_LINK: ";
    public final static String MISSING_PIN = "[GEOPHONE]MISSING_PIN";

}
